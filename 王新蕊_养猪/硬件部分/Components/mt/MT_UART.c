...
static void rxCB(uint8 port,uint8 event){
  unsigned char uartbuf[16];//读取串口数据
  ...
  */
  ...
  if (event & ( HAL_UART_RX_FULL | HAL_UART_RX_ABOUT_FULL | HAL_UART_RX_TIMEOUT))
  {
    if ( App_TaskID )
    {
      ...
      */
        //读取串口信息
    HalUARTRead( MT_UART_DEFAULT_PORT, (uint8 *)(msg_ptr + 1), rxBufLen );
   // HalUARTWrite( MT_UART_DEFAULT_PORT, (uint8 *)(msg_ptr + 1), rxBufLen);
    if(osal_memcmp((msg_ptr+1),"#1",2)){
      isRecvLed = 1;
      HalUARTWrite(MT_UART_DEFAULT_PORT,"#1\n",3);
    }
    else if(osal_memcmp((msg_ptr+1),"#2",2)){
      isRecvFan = 1;
      HalUARTWrite(MT_UART_DEFAULT_PORT,"#2\n",3);
    }
