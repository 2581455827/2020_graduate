...
byte fan_state = 1;//风扇默认为自动控制
byte led_state = 1;//led默认为自动控制
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  switch ( pkt->clusterId ) //判断簇ID
  {
    case SAMPLEAPP_PERIODIC_CLUSTERID: //收到广播数据
      ...
      /*
      unsigned char ledBuf[16]={0};
      sprintf(ledBuf,"led=%d\n",isRecvLed);
      HalUARTWrite(0,ledBuf,sizeof(ledBuf));
      */
      ...
      if((buf[0]=='#'&&buf[1]=='1')){
        //收到#1
        LED^=1;
        led_state = (LED==0)?0:1;//如果led灭，则禁止自动控制
        if(isRecvLed)
          isRecvLed = 0;
      }
      //收到#2,风扇开/关
        ...
        FAN^=1;
        fan_state = (FAN==0)?0:1;//如果风扇关，则禁止自动控制
        if(isRecvFan)
          isRecvFan = 0;
      }
#endif
