...
uint16 ReadTurData(void);
uint16 ReadPhData(void);
void myDelay(){
  for(int j =0;j<100;j++)
...
{ 
 ...
 //获取温度
  char tempStr[64]={0};
  char tempData[6]={0};
  HalUARTWrite(0,"AT",sizeof("AT"));
  HalUARTWrite(0,"\r\n",2);
  HalUARTRead(0,tempStr,64);0
#if 0
  int i = 0;
  while(tempStr[i]!=0xA1&&tempStr[i+1]!=0xE6){
    ++i;
  };
  if(i==63){
    osal_memcpy(tempData,"00.0",4);
//     HalUARTWrite(0,"00.0",4);
  }
  else{
      int j;
      i-=4;    
      for(j= 0;j<4;j++){
        //数据解析及检验
        /*
        if(tempStr[i+j]<'0'||tempStr[i+j]>'9'||tempStr[i+j]!='.')
          break;
        */
       tempData[j] = tempStr[i+j];
      }
      if(j<4){
        //如果捕获的数据是乱码,则将数据设置为00000
        osal_memset(tempData, 0 ,5);
      }
      tempData[4]='\0';
      //HalUARTWrite(0,tempData,5);
  }
 // HalUARTWrite(0,tempStr,64); 
  //HalUARTWrite(0,"\n",1); 
  //浊度
  uint16 turbidity = ReadTurData();
  // sprintf(str,"%4d",turbidity);
  //HalUARTWrite(0,str,4);  
  //ph值
  int ph = ReadPhData();
  ph = (ph-86)/2+7;
  sprintf(str,"%4s%3d%2d",tempData,turbidity,ph);
#endif
  ...
  if( AF_DataRequest( &SampleApp_Periodic_DstAddr,//发送目的地址＋端点地址和传送模式
                       &SampleApp_epDesc,//源(答复或确认)终端的描述（比如操作系统中任务ID等）源EP
                       SAMPLEAPP_PERIODIC_CLUSTERID, //被Profile指定的有效的集群号
                       sizeof(tempStr),       //发送数据长度
                       (uchar*)tempStr,// 发送数据缓冲区
                       &SampleApp_TransID,     // 任务ID号
                       AF_DISCV_ROUTE,      // 有效位掩码的发送选项
                       AF_DEFAULT_RADIUS ) == afStatus_SUCCESS )  //传送跳数，通常设置为AF_DEFAULT_RADIUS
  {
